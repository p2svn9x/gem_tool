...
<script>
    ...
    $(function () {
        ...
        $('#deleteManyBtn').click(function () {
            ...
            deleteToiChonTom(ids);
        })

    });
    function resultSearchtaixiu(stt, nickname, tom, van,thang,dat,phien,prize, id) {
        var rs = "";
        rs += "<tr>";
        ...
        rs += "<td>" + tom+ "</td>";
        rs += "<td>" + van + "</td>";
        rs += "<td>" + commaSeparateNumber(thang) + "</td>";
        rs += "<td>" + commaSeparateNumber(dat) + "</td>";
        rs += "<td>" + phien + "</td>";
        rs += "<td>" + prize + "</td>";
        rs += "<td><button onclick='deleteToiChonTom(\""+id+"\")' class='btn btn-sm btn-danger'>Xóa</button></td>";
        rs += "</tr>";
        return rs;
    }
    ...
    $("#search_tran").click(function () {
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo admin_url('event/toichontomajax')?>",
            data: {
                month: $("#txtmonth").val(),
                day: $("#txtday").val(),
                top: $("#top").val(),
            ...
            success: function (result) {
                ...
                if (result.results == "") {
                    ...
                    $.each(result.results, function (index, value) {
                        result += resultSearchtaixiu(stt, value.username, value.soCa, value.soVan, value.tongThang,value.tongDat,value.currentPhien,value.prize, value.id);
                        stt ++;
                    });
                                ...
                                $.ajax({
                                    ...
                                    success: function (result) {
                                        ...
                                        $.each(result.results, function (index, value) {
                                            ...
                                            result += resultSearchtaixiu(stt, value.username, value.soCa, value.soVan, value.tongThang,value.tongDat,value.currentPhien,prize, value.id);
                                            stt++;
//                                            result += resultSearchtaixiu(value.stt, value.nickname, value.hand, value.money, value.timestamp,prize, value.id);
                                        });
                                        $('#logaction').html(result);
    ...
    function deleteToiChonTom(ids) {
        if(!confirm('Bạn muốn xóa?')) return false;
        $("#spinner").show();
        $.post('<?php echo admin_url('event/deletetoichontomjax')?>', {
            ids: ids
        }, function (re) {
            $("#spinner").hide();
            if(re.success){
